...
+ What is the preload list?
  ...
  + This list is hardcoded into browsers, so a HTTP request to a domain can
    NEVER occur
    + Ex: Chrome Dev Tools ~307 Redirect~
  + Chromium project maintains the "master" list
    + Most major browsers HSTS preload lists inherit from the Chromium project's
      list
...
+ What happens if I dont use the preload list but I set the ~Strict-Transport-Security~ header?
  + The browser wont force HTTPS on the initial request
    + But after the browser recieves the header in the response, it will force HTTPS on all subsequent requests
...
+ We saw how hackthissite is leveraging HSTS to enforce encryption
  ...
  + This led to an unencrypted connection
    + This could allow a MiTM to inject malicious code on the page and steal the ~PHPSESSID~
    + Ex: ~PHPSESSID~'s lack of ~HttpOnly~ flag
+ Server Side validation Ex:
  + ~curl -v http://accounts.google.com/SignUp~
  + Ideally, should have this in addition to HSTS headers

** HSTS Assumption/Question
...
+ Whats wrong
  ...
  + Alarm: "Offload security tasks (or any validation) to a browser"
    + This is common practice
    + Browser can help, but there always needs to be server-side validation

** Bringing it all together
+ How does this relate to recon within censys.io?
+ Focus your searches on common sources of misconfiguration
  + HTTP Header misconfiguration
  + Leverage the aggregation tool to easily find descripencies/unusual behavior
** Censys search
